...
public class MySQL {
	...
	//class_id = -1はクラス区別なし(クラスの総和)
	public void insertSimulationL(double L[], int time){//L[K]
        ...
        try{
        	...
        	for(int i = 0; i < L.length; i++){
        		if(i == L.length -1) 
        			buf.append("("+combination_id+","+transition_id+",-1,"+i+","+L[i]+","+time+")");
        		else buf.append("("+combination_id+","+transition_id+",-1,"+i+","+L[i]+","+time+"),");
            }
            String sql = buf.toString();
            stmt.execute (sql);
        }
